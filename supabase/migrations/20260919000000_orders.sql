...
-- Safe to re-run: tables are created if missing, functions are replaced,
-- menu prices are upserted.
--
-- Security model
...
--     place_order(items, name) — prices every line from menu_items, never
...
-- the menu is readable by anyone; only staff (dashboard) can change it
...
create policy "Menu is public" on public.menu_items for select to anon, authenticated using (active);
...
begin
  ...
  from jsonb_to_recordset(p_items) as x (id text, qty int)
  left join public.menu_items m on m.id = x.id and m.active
  where m.id is null
  limit 1;
  ...
  -- prices come from the menu, never from the request
  ...
  join public.menu_items m on m.id = e.el ->> 'id'
